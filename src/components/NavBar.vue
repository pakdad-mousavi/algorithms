<template>
  <nav
    class="px-4 py-3.25 sm:py-2 border-b-[1px] border-neutral-800 fixed bg-zinc-900 w-full lg:w-[calc(100%-288px)] 2xl:w-[calc(100%-368px)] z-40 text-sm">
    <div class="flex items-center gap-4">
      <!-- Banner -->
      <div class="flex items-center mr-auto cursor-pointer gap-x-2 lg:hidden" @click="goToHomePage()">
      ...
      <!-- Breadcrumbs -->
      <div class="flex-1 hidden w-full lg:flex gap-x-2">
        <div class="flex items-center gap-x-2" v-for="(crumb, index) in algorithm">
          <span class="text-zinc-500 last:!text-white">{{ crumb }}</span>
          <Icon size="18px" v-if="index != algorithm.length - 1">
